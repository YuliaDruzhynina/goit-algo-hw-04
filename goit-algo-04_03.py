...
def get_upcoming_birthdays(users=None):
    tdate=datetimedatetime.today().date() # беремо сьогоднішню дату
    ...
    for user in users: # перебираємо користувачів
        ...
        bdate=datetimedatetime.strptime(bdate, "%Y.%m.%d").date() # перетворюємо дату народження в об’єкт date
        ...
        if 0<=days_between<7: # якщо день народження протягом 7 днів від сьогодні
            if week_day<6: #  якщо пн-пт
                ...
                if (bdate+datetimedatetime.timedelta(days=1)).weekday()==0:# якщо неділя
                    birthdays.append({'name':user['name'], 'birthday':(bdate+datetime.timedelta(days=1)).strftime("%Y.%m.%d")})
                    #Переносимо на понеділок. Додаємо запис у список.
                elif (bdate+datetime.timedelta(days=2)).weekday()==0: #якщо субота
                    birthdays.append({'name':user['name'], 'birthday':(bdate+datetime.timedelta(days=2)).strftime("%Y.%m.%d")})
                    #Переносимо на понеділок. Додаємо запис у список.
    return birthdays
